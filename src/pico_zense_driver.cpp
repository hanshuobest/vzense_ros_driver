...
namespace autolabor_driver
{
    ...
    bool PicoZenseDriver::configCamera()
    {
        ...
        status = Ps2_SetMapperEnabledRGBToDepth(_device_handle, _session_index, true);
        if (status != PsReturnStatus::PsRetOK)
        {
            std::cerr << "Ps2_SetMapperEnabledRGBToDepth failed: " << status << "\n";
            return false;
        }
    ...
    {
        if (_output_depth_image && ready_.mappedDepth &&
            Ps2_GetFrame(_device_handle, _session_index, PsMappedDepthFrame, &_depth_frame) == PsRetOK)
        {
